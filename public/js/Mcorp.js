...
var MSV = function (_MSV_) {
  ...
  var A = 2;
  var T = 3;
  var MsvCmd = {CREATE: 1, GET: 2, UPDATE: 3, DELETE: 4, PING: 5, PONG: 6, UNSUB: 7};
  var MsgType = {MESSAGE: 1, REQUEST: 2, RESPONSE: 3, EVENT: 4};
  (function () {
  ...
  var has_real_solution = function (p, v, a, x) {
    if (Math.pow(v, 2) - 2 * a * (p - x) >= 0) return true; else return false
  };
  var get_real_solutions = function (p, v, a, x) {
    if (a === 0 && v === 0) {
      if (p != x) return []; else return [0]
    }
    if (a === 0) return [(x - p) / v];
  ...
  var get_positive_real_solutions = function (p, v, a, x) {
    ...
    if (res.length === 0) return []; else if (res.length == 1) {
      if (res[0] > 0) {
        return [res[0]]
      } else return []
  ...
  var get_min_positive_real_solution = function (p, v, a, x) {
    ...
    if (res.length === 0) return null; else return res[0]
  };
  var calculate_delta = function (vector, pos_before, pos_after) {
    ...
    if (delta_before !== null && delta_after !== null) {
      if (delta_before < delta_after) return [delta_before, pos_before]; else return [delta_after, pos_after]
    } else if (delta_before !== null) return [delta_before, pos_before]; else if (delta_after !== null) return [delta_after, pos_after]; else return [null, null]
  };
  ...
  var urlparse = function (url) {
    ...
    if (url.substring(0, 6) != "msv://") {
      var e = {name: "MsvUrlError", message: "protocol not supported  " + url.substring(0, 6)};
      throw e
    }
...
var MSV = function (_MSV_) {
  ...
  var Tick = {SWITCH_MEDIUM: 3, SWITCH_LARGE: 10, SMALL_DELTA: 20, MEDIUM_DELTA: 500, LARGE_DELTA: 1e4};
  ...
  var IOState = {DISCONNECTED: 0, CONNECTING: 1, CONNECTED: 2};
  var IORemote = function (host, options) {
    ...
    var localStorage = function () {
      ...
      return {
        getItem: function (key) {
          if (!support) return undefined; else return storage.getItem(key)
    ...
    var send_ping = function () {
      var msg = {type: MsgType.REQUEST, cmd: MsvCmd.PING, data: MSV.client_clock()};
...
var MSV = function (_MSV_) {
  ...
  var makeMsvDB = function () {
    ...
    var create_msvs = function (msv_data_list) {
      for (var i = 0; i < msv_data_list.length; i++) {
        ...
        msvid_counter += 1;
        var msv_data = {msvid: msvid, eventid: 0, range: msv_data_list[i].range || [null, null]};
  ...
  var IOLocal = function () {
    ...
    var process_timeout = function (msvid, eventid, pos) {
      var args = [{msvid: msvid, eventid: eventid, vector: [pos, 0, 0]}];
      process_update(args)
    };
    var process_update = function (args) {
      ...
      var event = {type: MsgType.EVENT, cmd: MsvCmd.UPDATE, data: msv_data_list};
      output(event);
      return msv_data_list
    };
    var send = function (msg) {
      ...
      if (msg.cmd == MsvCmd.UPDATE) {
        ...
        response = {type: MsgType.RESPONSE, cmd: MsvCmd.GET, tunnel: msg.tunnel || null, data: db.get_msvs(msg.data)};
        output(response)
...
var MSV = function (_MSV_) {
  ...
  var requestManager = function (msvclient, io) {
    ...
    var do_request = function (msg) {
      ...
      var request = {msg: msg, tid: tid, sent: false};
  ...
  var msvclient = function (io, token, options) {
    ...
    var is_valid_update = function (msv_data) {
      ...
      try {
        for (var i = 0; i < 3; i++) {
          if (msv_data.vector[i] === null || msv_data.vector[i] === undefined) continue; else {
    ...
    var update = function (arg_list) {
      ...
      for (token in tokenMap) {
        msg = {cmd: MsvCmd.UPDATE, token: token, data: tokenMap[token]};
        if (server_version === "1") {
    ...
    var _get = function (msvid_list, token) {
      ...
      var msg = {type: MsgType.REQUEST, cmd: MsvCmd.GET, token: tokenToString(token), data: msvid_list};
    ...
    var proxy_update = function (msvid, p, v, a, options) {
      update([{msvid: msvid, vector: [p, v, a]}])
    };
  ...
  var msvinfo = function (v) {
    ...
    return {pos: v[MSV.P], vel: v[MSV.V], acc: v[MSV.A], ts: v[MSV.T]}
  };
  ...
  var msvproxy = function (msvclient, msvid) {
    ...
    var _callbacks = {readystatechange: [], change: [], timeupdate: [], error: [], skewchange: []};
    var _do_callbacks = function (what, e, handler) {
      ...
      for (var i = 0; i < _callbacks[what].length; i++) {
        ...
        if (handler === undefined) {
          ...
          if (h === handler) handler["_immediate_pending_" + what + msvid] = false; else {
            continue
          }
        }
        try {
    ...
    Object.defineProperty(api_public, "vector", {
      get: function () {
        ...
        return {position: v[P], velocity: v[V], acceleration: v[A], timestamp: v[T]}
      }
    });
    ...
    refresh_readystate();
    return {api_public: api_public, api_msvclient: api_msvclient}
  };
  _MSV_.msv = get_msv;
...
var MCorp = function (_mcorp_) {
  ...
  var STATE = Object.freeze({INIT: "init", USER: "user", CTX: "context", OPEN: "open", CLOSED: "closed"});
  var app = function (appid, options) {
    ...
    if (options.profile) {
      setTimeout(function () {
        ...
        var params = {profile: JSON.stringify(_profile), appid: appid, result: readystate.get()};
        JSONP.get(url, params)
      }, 6e3)
    }
    var _callbacks = {readystatechange: [], cancast: [], castbusy: []};
    var _do_callbacks = function (what, e, handler) {
      ...
      for (var i = 0; i < _callbacks[what].length; i++) {
        ...
        if (handler === undefined) {
          ...
          if (h === handler) handler._immediate_pending = false; else {
            continue
          }
        }
        try {
    ...
    var request_context = function (msvlist, onSuccess) {
      ...
      var ts_cs = (new Date).getTime(), ts_cr;
      var url = app_server + "/magic.py/get_context";
      var params = {app_id: appid, role: options.role, referrer: window.location.href};
      if (options.anon) {
        params.anon = true
      }
    ...
    var request_groupcontext = function (sid) {
      ...
      var params = {sid: sid, referrer: window.location.href};
      var onResult = function (result) {
    ...
    var get_association = function (key, onSuccess, onError) {
      ...
      var params = {id: key, referrer: window.location};
    ...
    var set_association = function (key, value, onSuccess, onError) {
      ...
      var params = {id: key, value: JSON.stringify(value), referrer: window.location};
      JSONP.get(url, params, onSuccess, onError)
    };
    var set_token = function (ctx_result, cs, cr) {
      ...
      _token = {appid: appid, ticket: ctx_result.t, expire: new Date(expire)}
    };
    ...
    var get_session = function (onSuccess, onError) {
      if (_sid) {
        onSuccess({result: "success", sid: _sid});
        return
      }
      var url = app_server + "/magic.py/get_session";
      var params = {app_id: appid, referrer: window.location};
      JSONP.get(url, params, onSuccess, onError)
    };
    var msvs_left;
    var profile_start;
    ...
    var profile_skew = function (msv) {
      ...
      if (_last_latency != info.latency) {
        ...
        _profile.skew.push({time: performance.now(), skew: info.skew, latency: info.latency})
      }
    ...
    self.create_global_motion = function (nick, mode) {
      return new Promise(function (resolve, reject) {
        ...
        if (!mode) mode = 1604;
        var params = {mode: mode, app_id: appid, nick: nick, referrer: window.location.href};
    ...
    self.ready = new Promise(function (resolve, reject) {
      self.run = function () {
        if (self.readyState !== self.STATE["OPEN"]) reject(); else resolve()
      };
      self.init()
    });
  ...
  var request_user_info = function (onSuccess, onError) {
    ...
    JSONP.get(url, {referrer: window.location.href}, onResult, onError)
  };
  var attribution = function (target, animate, nosync) {
    ...
    if (animate) {
      var logo_app = MCorp.app("5500231976530892295", {anon: true});
